	...
	public import_book() {
		...
		btnSale.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				int t2;
				try {
				    t2=Integer.parseInt(textField_1.getText());//number
				}catch(Exception e1) {
					WarningDialog warning = new WarningDialog();
			      	warning.WarningDialog("信息输入错误"); 
			      	return;
				}
				float t3;
				try {
					t3=Float.parseFloat(textField_2.getText());//price
					}catch(Exception e1) {
						WarningDialog warning = new WarningDialog();
				      	warning.WarningDialog("信息输入错误"); 
				      	return;
			...
			//	String t4=""+textField_3.getText();//order_ID
				...
				if(!t9.equals("")) {
					try {
						float t=Float.parseFloat(t9);//price
						}catch(Exception e1) {
							WarningDialog warning = new WarningDialog();
					      	warning.WarningDialog("信息输入错误"); 
					      	return;
