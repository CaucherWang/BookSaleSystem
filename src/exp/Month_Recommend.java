...
"jdbc:mysql://localhost:3306/bookshop?user=root&password=123456&useUnicode=true&useSSL=false&serverTimezone=GMT%2B8&characterEncoding=utf-8");  
	          ...
	          int i = 0,count = 0;
	          ResultSet rs = stmt.executeQuery("select ISBN,book_name,sum(number) as number, mon,price  from \r\n" + 
	          		"(select ISBN,number,book_name,extract(month from time) as mon,price from sales_bill natural join bill_order"
	          		+ " natural join bill natural join order_list natural join books)as p where mon='"+mon+"' group by ISBN order by number desc;");
	          while (rs.next()) { 
	        	  i ++;count++;
	        	  a1[a11]=i + "";
